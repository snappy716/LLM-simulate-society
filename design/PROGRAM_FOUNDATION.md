...
- JSON Schema 解析：通过。
- `git diff --check`：通过。
- 固定种子 42、规则模式 28 天历史验收及本轮 14 天回归：完整运行。
...
校园内核现已加入上午、下午、晚上、凌晨四个宽时段，以及玩家和每名 NPC 相互独立的主要行动余额：

- 每个时段默认只有 1 次主要行动；
- 推进时段会原子重置全部角色的主要行动余额；
- 从凌晨推进会进入次日上午并增加天数；
- 聊天、购物、吃饭和普通移动暂时都是自由行动，不扣时间和行动次数；
- `minute` 继续保留在命令和事件契约中，但当前只作为未来竞争排序的扩展点；
- `ADVANCE_PHASE` 与地点通行共用同一个校园命令接口，Godot 可直接调用；
- 校园灰盒右上角提供时段、主要行动余额和“结束当前时段”调试面板；
- 主要行动处理器必须通过 `consume_major_action` 扣除额度，第二次主要行动会在事务提交前被拒绝。

下一步应实现课程、工作、社团、阅读、深入调查等真实主要行动，并逐项接入这个统一预算，而不是直接修改余额。

## 12. 校园日程底座

当前还加入了数据驱动的一周日程底座：

- `content/actions/campus_schedules.json` 提供 12 套工作日/周末模板；
- 每个玩家或 NPC 记录保存七天四时段计划，跨周按星期重复；
- 计划地点必须开放且满足权限，并同时受室内地点和上级建筑容量限制；
- 超额申请按照优先级和固定种子稳定排序，失败者改派候补地点；
- Godot 校园快照 v3 公开当前计划和当前计划占用，灰盒面板显示玩家本时段计划；
- 计划不等于执行：当前不会让 NPC 瞬移，也不会在生成计划时扣主要行动。

这样下一步可以单独实现“依据日程选目标 → 统一寻路 → 到达后执行活动”，而不把日程、移动和活动结果混成一个不可测试的步骤。
