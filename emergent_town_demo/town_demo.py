...
from emergent_town.desires import DesireEngine
...
from emergent_town.plots.consequence_chains import ConsequenceChainEngine
from emergent_town.models import LongTermGoal, TraceEvidence
...
GAME_EVENT_TYPES = {
    ...
    "OCCULT_ITEM_FOUND",
    "ARREST_AUTHORIZED", "SUSPECT_ARRESTED", "SUSPECT_ESCAPED_ARREST",
    "HOSTILE_CELL_REGROUPED", "FUGITIVE_CAPTURED",
    "SUPPLEMENTARY_TESTIMONY_ATTACHED",
    "CROSS_FACTION_DEBT_CREATED",
}
...
class TraceLedger:
    ...
    def emit(self, *, day, phase, system, event_type, message,
             actor_ids=None, scene_id=None, payload=None,
             trace_id=None, parent_id=None):
        event_id = f"evt_{uuid.uuid4().hex[:12]}"
        trace_id = trace_id or f"trace_{uuid.uuid4().hex[:10]}"
        rec = {
            "event_id": event_id, "trace_id": trace_id, "parent_id": parent_id,
            "day": day, "phase": phase, "system": system,
            "event_type": event_type, "actor_ids": actor_ids or [],
            "scene_id": scene_id, "message": message, "payload": payload or {},
            "wall_time": time.strftime("%Y-%m-%d %H:%M:%S"),
        }
        with self.trace_jsonl.open("a", encoding="utf-8") as f:
            f.write(json.dumps(rec, ensure_ascii=False) + "\n")
        if event_type in GAME_EVENT_TYPES:
            game_rec = {key: rec[key] for key in (
                "event_id", "parent_id", "day", "phase", "event_type",
                "actor_ids", "scene_id", "message", "payload")}
            with self.game_events_jsonl.open("a", encoding="utf-8") as f:
                f.write(json.dumps(game_rec, ensure_ascii=False) + "\n")
        ...
        with self.world_log.open("a", encoding="utf-8") as f:
            f.write(line)
        for npc_id in actor_ids or []:
            with (self.log_dir / "npc_history" / f"{npc_id}.log").open("a", encoding="utf-8") as f:
                f.write(line)
        return rec

    def log_story_line(self, thread_id: str, line: str):
        with (self.log_dir / "story_threads" / f"{thread_id}.log").open("a", encoding="utf-8") as f:
            f.write(line.rstrip() + "\n")
...
class NPC:
    ...
    def relevant_memories(self, limit=8):
        ranked = sorted(self.memories, key=lambda m: (m.importance, m.day), reverse=True)
        return [m.summary for m in ranked[:limit]]
...
def make_scenes():
    ...
    return {x[0]: Scene(*x) for x in defs}


OCCUPATIONS = [
    ("文职员工", "blackthorn_security"),
    ("警察", "police_station"),
    ("教士", "evernight_church"),
    ("摊贩", "market"), ("杂货商", "market"), ("裁缝", "market"),
    ("酒保", "tavern"), ("酒馆侍者", "tavern"),
    ("医生", "hospital"), ("护士", "hospital"), ("药剂师", "hospital"), ("护工", "hospital"),
    ("码头工人", "east_dock"), ("码头调度员", "east_dock"), ("货运书记员", "east_dock"),
    ("工厂工人", "factory"), ("机械师", "factory"), ("车间主管", "factory"),
    ("大学教师", "university"), ("大学生", "university"), ("图书管理员", "university"),
    ("记者", "newspaper"), ("编辑", "newspaper"), ("印刷工", "newspaper"),
    ("占卜师", "divination_club"), ("俱乐部接待员", "divination_club"),
    ("银行柜员", "bank"), ("会计", "bank"), ("银行警卫", "bank"),
    ("私家侦探", "detective_agency"), ("侦探助理", "detective_agency"),
    ("歌剧演员", "opera_house"), ("乐师", "opera_house"), ("舞台工", "opera_house"), ("售票员", "opera_house"),
...
def build_work_schedule(idx, work_scene, occupation):
    """Return stable weekday indexes and shifts; residents do not share one global timetable."""
...
def make_npc(idx, tier, rng):
    ...
    npc = NPC(
        id=f"npc_{idx:03d}", name=name, tier=tier, occupation=occupation,
        ...
        states={"energy":rng.randint(60,95),"satiety":rng.randint(55,95),
                "stress":rng.randint(5,30),"fear":rng.randint(0,20),
                "pain":0,"alertness":rng.randint(35,85),
                "morale":rng.randint(45,85),"loneliness":rng.randint(5,45),
                "reputation":50,"legal_risk":0,"social_support":rng.randint(20,70),
                "civic_duty":rng.randint(20,90)},
        wealth=rng.randint(20,100), sanity=rng.randint(65,95)
    )
    npc.goals = [f"维持自己的{occupation}生活",
        ...
        "plans":{
            ...
            "properties":{
                p.value:{
                    ...
                    "required":["scene_id","intent","target_id","priority","behavior","fallback_scene_id"]
                } for p in PHASES
            },
            "required":[p.value for p in PHASES]
        },
        "strategy_steps":{"type":"array","maxItems":3,"items":{"type":"object",
            "properties":{"phase":{"type":"string"},"action_id":{"type":"string"},
                          "scene_id":{"type":"string"},"target_id":{"type":["string","null"]},
                          "condition":{"type":"string"},"intent":{"type":"string"}},
            "required":["phase","action_id","scene_id","target_id","condition","intent"]}}
    },
...
def add_mutual_relationship(a, b, kind, trust=50, affection=0, suspicion=0, fear=0):
    """Create or update both perspectives of a genuinely mutual social tie."""
...
def build_initial_relationship_network(npcs, rng):
    ...
    for colleagues in by_workplace.values():
        ...
        for index, npc in enumerate(colleagues):
            if len(colleagues)>1:
                add_mutual_relationship(npc,colleagues[(index+1)%len(colleagues)],
                                        "同事",trust=58,affection=12)

    shuffled=residents[:]
    rng.shuffle(shuffled)
    for index,npc in enumerate(shuffled):
        add_mutual_relationship(npc,shuffled[(index+1)%len(shuffled)],
    ...
    for index in range(0,len(candidates)-1,2):
        ...
        if "朋友" not in a.relationships.get(b.id,Relationship()).kinds:
            add_mutual_relationship(a,b,"仇人",trust=8,affection=-65,suspicion=75,fear=12)
    ...
    while cursor+1<len(former_candidates):
        ...
        cursor+=2
        if a.work_scene!=b.work_scene and "仇人" not in a.relationships.get(b.id,Relationship()).kinds:
            add_mutual_relationship(a,b,"前同事",trust=50,affection=8,suspicion=8)
...
def restock_essential_supplies(world):
    """Model daily local production so a long run cannot permanently exhaust the town."""
    supplies=[
    ...
    for object_id,amount,capacity,reason in supplies:
        ...
        if obj.quantity!=old:
            world.ledger.emit(day=world.day,phase="night_resolution",system="supply_system",
                              event_type="SUPPLY_RESTOCKED",
                              message=f"{reason}：{obj.name} 从 {old} 补充到 {obj.quantity}。",
                              scene_id=obj.scene_id,payload={"object_id":object_id,
                              "old_quantity":old,"new_quantity":obj.quantity})
...
def save_snapshot(world):
    ...
    (world.log_dir/f"snapshot_day_{world.day:03d}.json").write_text(
