...
保存并应用只会切换当前认知接口，不会立即调用外部服务。接口从下一个校园时段开始，仅供当前深度 NPC 从规则层生成的合法候选中选择。每时段最多 4 次、每天最多 12 次；超时、额度耗尽或非法响应立即回退本地规则。
...
该进程在内存中保持 Python 世界对象，并只监听 `127.0.0.1:8765`。默认使用离线规则规划，不会因为打开项目而访问外部 LLM。兼容接口的 API Key 只保存在进程内存和 Godot 本机配置，不进入模拟存档、事件或 Git。

新校园 NPC 认知层的详细设计与模型预设见 `design/NPC_COGNITION_LLM_RUNTIME.md`。正式联网测试还需要服务方的 API Base URL。
